jQuery(document).ready(function(){
		...
		// SMOOTH SCROLL
		jQuery('#menu-nav ul li a').on(function(){
   			jQuery('#menu-nav').removeClass('in');
  		});
		jQuery('.nav li a').on('click', function(e) {
            e.preventDefault();
            jQuery('html,body').animate({scrollTop: jQuery(this.hash).offset().top - 68});                                                         
        });
		...
		simpleCart.on( 'beforeAdd' , function( item ){
			//alert("modal");
			jQuery('.modal').modal({
				backdrop:	true,
			});
